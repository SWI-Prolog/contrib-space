...
%
% Converts the contents of an KML file into GeoRSS RDF in the RDF
% database of Prolog.
%
% The Geometries are converted to GeoRSS properties and values.
% Documents, Folders, etc. are ignored.  MultiGeometry objects are
% expanded into separate simple Geometries.  Geometries with an XML ID
% are assigned that ID as URI, other Geometries are assigned a RDF
% blank node.  The kml:name and kml:description are translated to RDF
% properties.

kml_file_to_georss(File) :-
  findall(Res-Shape, kml_file_uri_shape(File, Res, Shape), Pairs),
  forall(member(Res-Shape, Pairs), assert_georss(Res, kml:dummy, Shape, File)).
...
%! assert_georss(+S, +P, +Shape) is det.
%! assert_georss(+S, +P, +Shape, ?G) is det.
%
% Converts between a Res-Shape pair and its GeoRSS simple RDF triple
% form.

assert_georss(S, P, Shape, G) :-
  functor(Shape, Type, _),
  string_phrase(coords(Shape), Lex),
  rdf_global_id(georss:Type, D),
  rdf_assert(S, P, Lex^^D, G).
